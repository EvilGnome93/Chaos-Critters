...
- **26 pet-soorten in `scripts/seed.py`** (3 tiers: Common 70%, Rare 25%, Legendary 5%, gelijk verdeeld binnen tier). De eerste 13 hebben elk een eigen AI-gegenereerde afbeelding in `docs/assets/`, gelinkt via `scripts/link_afbeeldingen.py` (leest repo-naam automatisch uit de git remote, dus overleefde de repo-rename zonder handwerk). **13 nieuwe soorten toegevoegd (2026-07-21)** — Gans/Marter/Chaos Rat/Eekhoorn/Hagedis/Kever (T1), Valk/Hert/Chaos Bever/Hermelijn (T3), Lynx/Ringslang/Chaos Zwijn (T5): naam+tier kwamen van de gebruiker, gevecht/werk-stats+werkplek+beschrijving zijn door mij ingevuld (zelfde patroon: omgekeerde correlatie gevecht/werk, Legendary zonder werkplek-voorkeur). **Nog te doen**: `python scripts/seed.py` opnieuw draaien tegen de live DB om ze daadwerkelijk toe te voegen (seeding gebeurt niet automatisch bij opstart, alleen migraties) — en afbeeldingen genereren + `scripts/link_afbeeldingen.py` draaien, anders tonen ze de placeholder.
- **5 werkplekken geseed**, elk gekoppeld aan een eigen grondstof-item: Moestuin→Groente, Vijver→Algen, Werkbank→Schroot, Bos→Takken, Nachtwacht→Maanschijnkristal (hernoemd 2026-07-21, was Moestuin-oogst/Vijver-extract/Werkbank-materiaal/Bos-oogst/Nachtwacht-extract — via migratie `9d3f2b6a7c41`, want de oude namen stonden al in de live DB).
...
- **`/tests`** (admin) — stuurt een `@everyone`-oproep (embed) met een korte uitleg per commando (`/vang`, `/lijst`, `/werk`, `/verzorg`, `/shop`, `/items`, en de admin-commando's `/spawn` + `/give` — die laatste 2 expliciet erbij omdat de testgroep momenteel allemaal admin is), om een testronde aan te kondigen. Lijst met commando's staat hardcoded in `TEST_COMMANDOS` in `cogs/admin.py` — bijwerken als er een nieuw commando bijkomt. Vereist dat de bot-rol de "Mention @everyone"-permissie heeft, anders pingt het niet.
